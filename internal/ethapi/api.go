...
func (s *BlockChainAPI) EstimateGas2(ctx context.Context, args []TransactionArgs,
...
func DoManyCall(ctx context.Context, b Backend, args []TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash,
	overrides *StateOverride, timeout time.Duration, globalGasCap uint64) ([]uint64, []error, error) {
	defer func(start time.Time) { log.Debug("Executing EVM call finished", "runtime", time.Since(start)) }(time.Now())
	state, header, err := b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, nil, err
	...
	for _, a := range args {
		...
		res, _ := core.ApplyMessage(evm, msg, gp)
